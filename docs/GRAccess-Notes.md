...
- A device's scan group has an `ItemList` attribute, but it is not a complete list of what is assigned to it (on EMGALAXY `GPM_actual` and `GPM_SP` of `LSC3_PumpVFDControl_CHWR` are assigned but not listed). It is a string array: read it with `value.GetDimensionSize(out size)` and `value.GetElement(i, element)` for `i` from 1 to `size`. Compare items without case, like all ArchestrA names: the list has `LSC3_PumpVFDControl_CHWR.CMD` for the `Cmd` attribute.
