...
- `HierarchicalName` (`gobject.hierarchical_name`) is read-only through GRAccess. The galaxy rebuilds it as `<container's hierarchical name>.<contained name>` every time the contained name changes, so a stale name is fixed by setting `ContainedName` to a temporary value and back: setting it to the value it already has succeeds but changes nothing, and `Container = <current container>` fails with "Object is already contained by this container". A deployed object is refused with "This Object is currently deployed", so undeploy it first. `Container = ""` takes an object out of its container and resets its contained name to its tagname; while an object has no container, setting a contained name fails with "Cannot rename top level application objects".
- Taking a container out of its own container does cascade into its children (TESTGALAXY: `ZZRen_G.ZZRen_M.ZZRen_C` became `ZZRen_M.ZZRen_C`), but two EMGALAXY set point controls had kept the old path anyway (`LSCP_CoolingTower_1.LSCP_ValveAO_CWR_1.SetPointControl` under a top-level `LSCP_ValveAO_CWR_1`). Because the auto I/O item name is built from the hierarchical name, all of their points were unmapped, and rebuilding the name mapped them.
...
## Creating instances and assigning area and I/O (verified on TESTGALAXY and EMGALAXY)

- `((ITemplate)template).CreateInstance(name, true)` creates the instance and its contained objects, checked in (configuration version 1), not deployed, with no area or host. Check the template's `CommandResult` and that the returned `IInstance` is not null. Tagnames are at most 32 characters (the database procedure takes `nvarchar(32)`).
- Contained objects get generated tagnames from the new instance's name: `ECCP_CH7_ValveAO_001` (hierarchical `ECCP_CH7_ValveAO.SetPointControl`), `ZZAddInst_T2_001` and `_002`. Objects created in the IDE were named from the contained name instead (`SetPointControl_001` in TrainingGalaxy).
- Assign an application object to an area with `obj.Area = "<area>"`: it needs no check-out, is immediate, sets the host to the area too, and moves contained objects along. `obj.Host = "<area>"` fails with "Invalid host assignment", although the GRAccess help says an application object's area must be set through its host.
- A new object with `---Auto---` I/O references can be assigned to a scan group as soon as it is created, before any area: on TESTGALAXY every such `$ValveAO` and `$SteamChiller.ChillerValve` went to `Simulator.Fast` (the package server falls back to "Simulator" and "Fast" when the user has no default area with an I/O device). On EMGALAXY the `$ValveAO` parent took the area's scan group when its area was set, but its SetPointControl did not and needed the IDE's call below (the tool did not yet record which scan group it was on). Setting the area copies the area's own assignment (`object_device_linkage` of the area) only to objects that have none, so objects already on Simulator.Fast stay there.
- GRAccess has no documented call to assign I/O. The IDE's "assign to IO device" is `IGalaxyConfigurationV25.AssignObjectsToDIOScanGroup(EASSIGNTYPE.eMyLinkedDevice, <device gobject_id>, <object gobject_ids>, EIDEVIEW.eModelView, <scan group mx_primitive_id>, out FAILURESTATUS[] failures)` on the object `IGalaxy.GetGalaxyConfiguration()` returns; `GetScanGroupInfo(<device id>, out SCANGROUPINFO[], out string)` gives the scan group ids. The types are in `ArchestrA.Configuration.dll` and `ArchestrA.Core.dll`. Both are in the GAC twice with the same assembly name (1.0.0.0): a .NET 4 program loads the `%WINDIR%\assembly\GAC_MSIL` copies (file version 2850), not the `%WINDIR%\assembly\GAC` ones (.NET 1.1 builds, file version 3650, with a few more interfaces), so compile against `GAC_MSIL` to use only what is there at runtime; `ArchestrA.Core` also defines `MxValue`, `EConditionType`, `ECATEGORY` and more, so alias its types instead of `using ArchestrA.Core;`. It needs no IDE running. Verified: it moved objects from Simulator.Fast, from the area's scan group to another one, and into an area without an I/O device; list every contained object explicitly. It wrote the same `object_device_linkage` and `autobound_attribute` rows as an IDE assignment and did not change the device's version or package. `AddInstances` uses it and checks the result in the database.
- Area and I/O assignments are not written to `gobject_change_log` (only `CreateInstance` is), so a tool has to record before and after values itself.
